...
function StatCard({ icon: Icon, color, value, label, fact, delay }: {
  ...
  return (
    <motion.div
      ...
      className="bg-secondary border border-border rounded-2xl p-6 text-center"
    >
      <div className={`w-12 h-12 rounded-2xl ${color} flex items-center justify-center mx-auto mb-4`}>
        <Icon className="w-6 h-6 text-foreground" />
      </div>
      <div className="text-4xl font-black text-foreground mb-1">{value}</div>
      <div className="text-sm text-muted-foreground mb-3">{label}</div>
      {fact && <p className="text-xs text-muted-foreground italic leading-relaxed">{fact}</p>}
    </motion.div>
  );
}
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-10">
      ...
      <motion.div
        ...
        className="relative overflow-hidden bg-gradient-to-br from-violet-600/20 via-indigo-600/15 to-amber-500/10 border border-border rounded-3xl p-8 text-center"
      >
        ...
        <div className="relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-secondary border border-border text-muted-foreground text-xs font-semibold px-3 py-1.5 rounded-full mb-4">
        ...
        >
          {data.topCareer && (
            <div className="bg-secondary border border-border rounded-2xl p-5 flex items-center gap-4">
              ...
              <div>
                <div className="text-xs text-muted-foreground mb-0.5">Most explored career</div>
                <div className="font-bold text-foreground text-lg">{data.topCareer}</div>
                <div className="text-xs text-muted-foreground">You came back to this one again and again ⭐</div>
              </div>
            </div>
          )}
          ...
          {data.atsScore !== null && (
            <div className="bg-secondary border border-border rounded-2xl p-5 flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="text-xs text-muted-foreground mb-1">Your CV ATS Score</div>
                <div className="flex items-center gap-3">
                  <div className="text-3xl font-black text-indigo-700">{data.atsScore}</div>
          ...
          {data.activeAlerts > 0 && (
            <div className="bg-secondary border border-border rounded-2xl p-5 flex items-center gap-4">
              ...
              <div>
                <div className="text-xs text-muted-foreground mb-0.5">Active job alerts</div>
                <div className="font-bold text-foreground">{data.activeAlerts} alert{data.activeAlerts !== 1 ? "s" : ""} running</div>
                <div className="text-xs text-muted-foreground">Opportunities are coming to you daily 📬</div>
              </div>
            </div>
          )}
        </motion.div>
      ...
      {step >= 5 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="bg-gradient-to-r from-violet-600/20 to-indigo-600/20 border border-violet-500/20 rounded-2xl p-6 text-center">
            <h3 className="font-bold text-foreground text-lg mb-1">Make {year + 1} your breakthrough year</h3>
