...
const LEVEL_LABELS: Record<number, { label: string; color: string; pct: string }> = {
  7: { label: "Outstanding",    color: "text-emerald-700", pct: "80–100%" },
  6: { label: "Meritorious",    color: "text-indigo-700",  pct: "70–79%"  },
  5: { label: "Substantial",    color: "text-violet-700",  pct: "60–69%"  },
  4: { label: "Adequate",       color: "text-amber-700",   pct: "50–59%"  },
  3: { label: "Moderate",       color: "text-orange-700",  pct: "40–49%"  },
  2: { label: "Elementary",     color: "text-red-700",     pct: "30–39%"  },
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="max-w-2xl mx-auto px-4 py-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 text-amber-700 text-xs font-semibold px-3 py-1.5 rounded-full mb-5">
            <GraduationCap className="w-3.5 h-3.5" />
            Free Matric Career Matcher — No sign-up required
          </div>
          <h1 className="text-4xl sm:text-5xl font-bold mb-4 leading-tight">
            What careers do<br />
            <span className="text-primary">your matric results</span><br />
            unlock?
          </h1>
          <p className="text-muted-foreground text-lg">Enter your symbols. We&apos;ll show you your APS score, matching careers, and university programmes you qualify for.</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-sm font-semibold text-foreground">Enter your achievement levels (1–7)</h2>
            <div className="text-right">
              <div className="text-2xl font-bold text-amber-700">{aps}</div>
              <div className="text-[10px] text-muted-foreground">APS score</div>
            </div>
          </div>

          {/* Level legend */}
          <div className="grid grid-cols-4 gap-1 text-center bg-secondary rounded-xl p-2">
            {[7,6,5,4,3,2,1].slice(0,4).map(l => (
              <div key={l} className="text-[10px]">
                <span className={`font-bold ${LEVEL_LABELS[l].color}`}>{l}</span>
                <span className="text-muted-foreground ml-1">{LEVEL_LABELS[l].pct}</span>
              </div>
            ))}
          </div>
          ...
          {/* Maths choice */}
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-2">Mathematics type</label>
            ...
            <div key={subject.id}>
                ...
                <div className="relative mb-1">
                  <select
                    ...
                    className={`w-full appearance-none border rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-indigo-500/40 pr-7 transition-colors ${
                      subjectNames[subject.id]
                        ? "bg-secondary border-indigo-500/30 text-foreground"
                  ...
                  >
                    ...
                    {ELECTIVE_SUBJECT_GROUPS.map(group => (
                      <optgroup key={group.group} label={group.group} className="bg-card text-muted-foreground text-[10px]">
              ...
              <div className="flex items-center gap-2">
                <div className="w-44 text-xs flex-shrink-0">
                  {subject.id === "maths"
                    ? <span className="text-muted-foreground">{mathsChoice}</span>
                    : ["subject4","subject5","subject6"].includes(subject.id)
                    ? subjectNames[subject.id]
                      ? <span className="text-muted-foreground">{subjectNames[subject.id]}</span>
                      : <span className="text-amber-700/60 italic">Select subject above ↑</span>
                    : <span className="text-muted-foreground">{subject.label}</span>}
                  {!subject.countForAps && <span className="text-muted-foreground ml-1">(excl. APS)</span>}
                </div>
                    ...
                    <button key={level} onClick={() => setGrade(subject.id, level)}
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${
                        grades[subject.id] === level
                          ? level >= 5 ? "bg-emerald-500/30 text-emerald-700 border border-emerald-500/50"
                          : level >= 4 ? "bg-amber-500/30 text-amber-700 border border-amber-500/50"
                          : "bg-red-500/20 text-red-700 border border-red-500/40"
                          : "bg-secondary text-muted-foreground hover:bg-secondary hover:text-foreground border border-transparent"
                      }`}>
                      {level}
                    </button>
                  ))}
          ...
          <button onClick={() => { setShowResults(true); track("tool_matric_run"); }} disabled={!allFilled}
            className="w-full py-3.5 rounded-xl font-semibold text-sm transition-all bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 disabled:opacity-30 disabled:cursor-not-allowed text-foreground flex items-center justify-center gap-2 mt-2">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-6 space-y-5">
              ...
              <div className={`rounded-2xl p-5 border ${
                ...
                <div className="flex items-center gap-3 mb-2">
                  <Star className={`w-5 h-5 flex-shrink-0 ${aps >= 36 ? "text-emerald-700" : aps >= 26 ? "text-indigo-700" : "text-amber-700"}`} />
                  <span className="font-bold text-xl text-foreground">APS Score: {aps} / 42</span>
                </div>
                    ...
                    {qualifyingProgrammes.map(p => (
                      <div key={p.programme} className="flex items-start gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <span className="text-muted-foreground ml-2 text-xs">{p.university}</span>
                          <span className="text-muted-foreground ml-2 text-xs">Min APS: {p.minAps}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {matchingCareers.length > 0 && (
                <div className="bg-secondary border border-border rounded-2xl p-5">
                  ...
                  <p className="text-xs text-muted-foreground mb-4">Based on the subjects you&apos;re taking — you have the prerequisites for these roles.</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {matchingCareers.map(c => (
                      <div key={c.id} className="bg-secondary rounded-xl px-3 py-2.5 flex items-center justify-between">
                        <div>
                        ...
                        <div className="text-right flex-shrink-0">
                          <div className="text-xs font-bold text-emerald-700">R{Math.round(c.avgSalaryZar/1000)}k</div>
