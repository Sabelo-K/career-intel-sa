...
export function SalaryProbe() {
  ...
  const verdictTone =
    band.verdict === "underpaid"
      ? { pill: "bg-[#A5423F]/20 text-destructive border-[#A5423F]/45", label: "Underpaid" }
      : band.verdict === "overpaid"
        ? { pill: "bg-indigo-500/20 text-indigo-700 border-indigo-500/50", label: "Above market" }
        : band.verdict === "fair"
          ? { pill: "bg-emerald-500/15 text-emerald-700 border-emerald-500/40", label: "Market rate" }
          : { pill: "bg-secondary text-muted-foreground border-border", label: "Add your salary" };

  return (
    /* Glass lives here, on the shell. Nothing inside re-blurs on keystroke. */
    <div className="glass-card rounded-2xl overflow-hidden">
      <div className="flex items-center gap-2 px-4 sm:px-5 py-2.5 border-b border-border">
        <span className="text-[11px] font-mono uppercase tracking-[0.14em] text-muted-foreground">
          Free salary benchmark
        </span>
        <span className="ml-auto flex items-center gap-1.5 text-[11px] font-mono uppercase tracking-[0.12em] text-emerald-700">
      ...
      <div className="p-4 sm:p-6 lg:p-7 grid gap-6 lg:grid-cols-[290px_1fr] lg:gap-8">
        ...
        <div className="lg:order-2 flex flex-col gap-5">
          <div className="flex items-start gap-4 flex-wrap">
            <div className="flex-1 min-w-[200px]">
              <p className="text-[11px] font-mono uppercase tracking-[0.14em] text-muted-foreground">
                Market rate · <span className="text-indigo-700">{role.title}</span>
              </p>
              <div
          ...
          <div>
            ...
            >
              <div className="absolute inset-x-0 top-[26px] h-3 rounded-full border border-border bg-gradient-to-r from-indigo-500/20 via-indigo-500/55 to-[#A5423F]/45" />

              <div className="absolute top-[18px] w-px h-7 bg-secondary" style={{ left: 0 }}>
                <span className="absolute top-8 left-0 text-[11px] font-mono text-muted-foreground whitespace-nowrap">
              ...
              >
                <span className="absolute top-8 left-1/2 -translate-x-1/2 text-[11px] font-mono text-indigo-700 whitespace-nowrap">
                  {formatZarShort(band.avg)} avg
                </span>
              </div>
              <div className="absolute top-[18px] w-px h-7 bg-secondary" style={{ right: 0 }}>
                <span className="absolute top-8 right-0 text-[11px] font-mono text-muted-foreground whitespace-nowrap">
              ...
              {band.current !== null && (
                ...
                >
                  ...
                  <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[11px] font-mono text-amber-700 whitespace-nowrap">
            ...
            <div className="flex flex-col gap-2">
              {ladder.map((p) => (
                <div key={p.code} className="flex items-center gap-3 text-sm">
                  <span
                    className={`w-[104px] shrink-0 truncate ${p.code === province ? "text-indigo-700 font-medium" : "text-muted-foreground"}`}
                  >
                    {p.name}
                  </span>
                  ...
                  <span className="w-[74px] text-right font-mono text-xs tabular-nums text-muted-foreground">
          ...
          {band.verdict && (
            <div className="flex items-center gap-3 flex-wrap rounded-xl border border-[#A5423F]/30 bg-[#A5423F]/10 px-4 py-3">
              <p className="flex-1 min-w-[190px] text-sm text-destructive">
                {band.verdict === "underpaid"
              ...
              <button
                type="button"
                onClick={onShare}
                className="flex items-center gap-2 px-3.5 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:border-border hover:bg-secondary transition-colors"
              >
        ...
        <div className="lg:order-1 flex flex-col gap-4">
          <div ref={wrapRef} className="relative">
            <label
              htmlFor="probe-role"
              className="block text-[11px] font-mono uppercase tracking-[0.14em] text-muted-foreground mb-2"
            >
              Your role
            </label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-secondary border border-border rounded-lg pl-9 pr-3 py-2.5 text-sm text-primary-foreground placeholder:text-muted-foreground focus:outline-none focus:border-indigo-500/60 focus:bg-indigo-500/5 transition-colors"
              />
            </div>
            ...
            {open && query.trim().length >= 2 && (
              <div
                ...
                className="absolute z-30 left-0 right-0 mt-1.5 max-h-[228px] overflow-y-auto rounded-lg border border-border bg-card shadow-2xl shadow-black/60"
              >
                {suggestions.length > 0 ? (
                  suggestions.map((c, i) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 border-b border-border last:border-0 transition-colors ${
                        i === cursor ? "bg-indigo-500/25 text-indigo-100" : "text-muted-foreground hover:bg-secondary"
                      }`}
                    >
                  ...
                  <div className="px-3 py-3">
                    ...
                    <p className="mt-1 text-[11px] text-muted-foreground">
                      ...
                      <Link href="/job-market" className="text-indigo-700 hover:text-indigo-700 underline">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-secondary border border-border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:border-indigo-500/60 transition-colors"
            >
              {PROVINCES.map((p) => (
          ...
          <div>
            <span className="block text-[11px] font-mono uppercase tracking-[0.14em] text-muted-foreground mb-2">
              Experience
            </span>
            <div className="flex rounded-lg border border-border overflow-hidden" role="group">
              {EXPERIENCE_BANDS.map((b) => (
                <button
                  ...
                  className={`flex-1 px-1 py-2.5 text-xs border-r border-border last:border-r-0 transition-colors ${
                    experience === b.value
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-secondary border border-border rounded-lg px-3 py-2.5 text-sm text-primary-foreground placeholder:text-muted-foreground focus:outline-none focus:border-indigo-500/60 focus:bg-indigo-500/5 transition-colors"
            />
          </div>

          <Link
            href="/sign-up"
            className="group flex items-center justify-center gap-2 mt-1 px-4 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-primary-foreground text-sm font-semibold transition-all hover:shadow-lg hover:shadow-indigo-500/25"
          >
